...
class App {
  ...
  onTouchMove(e) {
    if (!this.active || !this.isDown) return;
  ...
  update() {
    // Off-screen: skip ALL work (lerp, media updates, GPU render) — the
    // reel stays mounted on the homepage permanently, so only pay when
  ...
  setActive(active) {
    this.active = active;
    // Clear any in-progress drag when the reel leaves view so a stale
    // isDown can't capture a future pointermove after re-activation.
    if (!active) {
      this.isDown = false;
    }
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      // so the reel would drift even when scrolled far off-screen. Only
      // respond to input while the gallery is actually visible.
      const observer = new IntersectionObserver(
        entries => {
          app?.setActive(entries[0]?.isIntersecting ?? false);
        },
        { rootMargin: '200px' }
      );
